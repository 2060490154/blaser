        ...
        */
        ...
        public:
            ...
            */
            ...
            /*!
             \brief
                Creates a \c CVariant of type \c VariantDataType_PylonImage. The image will be allowed to be modified by the recipe.
            ...
            */
            ...
            /*!
             \brief
                Creates a \c CVariant of type \c VariantDataType_PylonImage and lets you specify if the image can be
                modified by the recipe.
             \param[in] value The value to assign.
             \param[in] readOnly If true the image must not be modified by the recipe and false otherwise.

             \post
                <ul>
                    <li>The \c value passed is shallow-copied (see \c CPylonImage copy constructor for more information)
                        and held by the \c CVariant created.</li>
                    <li>\c GetDataType() returns \c VariantDataType_PylonImage.</li>
                </ul>
             ...
             \error
                Doesn't throw C++ exceptions.
            */
            CVariant(const CPylonImage& value, bool readOnly);

            /*!
             \brief
                Creates a \c CVariant of type \c VariantDataType_PylonImage from a grab result with image payload and lets
                you specify if the image can be modified by the recipe.

             \param[in] ptrGrabResult The grab result that contains the image.
             \param[in] readOnly If true the image must not be modified by the recipe and false otherwise.

             \pre
                <ul>
                    <li>\c ptrGrabResult must be valid.</li>
                    <li>The grab result must be successful.</li>
                    <li>The payload of the grab result must contain an uncompressed image (and not contain GenDC data).</li>
                    <li>The pixel type must not be undefined.</li>
                </ul>

             \post
                <ul>
                    <li>The \c ptrGrabResult passed is shallow-copied (so no image data is copied) and held by the \c CVariant created.</li>
                    <li>\c GetDataType() returns \c VariantDataType_PylonImage.</li>
                </ul>
             ...
             \error
                Throws an exception if the preconditions aren't met.
            */
            CVariant(const CGrabResultPtr& ptrGrabResult, bool readOnly = false);

            /*!
             \brief
                Creates a \c CVariant of type \c VariantDataType_PylonImage from a grab result with GenDC payload that contains an image
                in the specified data component.

             \param[in] componentIndex The index of the data component that contains the image.
             \param[in] ptrGrabResult The grab result that contains the GenDC data.

             \pre
                <ul>
                    <li>\c ptrGrabResult must be valid.</li>
                    <li>The grab result must be successful.</li>
                    <li>The payload type of the grab result must be \c PayloadType_GenDC.</li>
                    <li>\c componentIndex must be a valid index of a data component that contains an image.</li>
                </ul>
             ...
             \error
                Throws an exception if the preconditions aren't met.
            */
            CVariant(size_t componentIndex, const CGrabResultPtr& ptrGrabResult);
            ...
            */
            ...
            /*!
             \brief
                Changes the value referenced by the variant. The image will be allowed to be modified by the recipe.
            ...
            */
            ...
            /*!
             \brief
                Changes the value referenced by the variant and allows to specify if the image may be modified by the recipe.
             ...
             \error
                Throws an exception if the preconditions aren't met.
            */
            void FromImage(const CPylonImage& newValue, bool readOnly);

            /*!
             \brief
                Changes the value referenced by the variant with a grab result with image payload and lets
                you specify if the image can be modified by the recipe.
             ...
             \post
                <ul>
                   <li>The \c ptrGrabResult passed is shallow-copied (so no image data is copied) and held by the \c CVariant now.</li>
                   <li>The data type returned by \c GetDataType() hasn't changed.</li>
                </ul>
             ...
             \error
                Throws an exception if the preconditions aren't met.
            */
            void FromGrabResult(const CGrabResultPtr& ptrGrabResult, bool readOnly = false);

            /*!
             \brief
                Changes the value referenced by the variant (the image must not be modified by the recipe) with a grab
                result with GenDC payload that contains an image in the specified data component.
             ...
             \error
                Throws an exception if the preconditions aren't met.
            */
            void FromGrabResult(size_t componentIndex, const CGrabResultPtr& ptrGrabResult);

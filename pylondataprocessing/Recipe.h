        ...
        */
        ...
        public:
            ...
            */
            ...
            /*!
             \brief
                ...
                Calls \c DeallocateResources() if the design described by the recipe has allocated resources. Unregisters all observers that have been connected and stops the measurement
                if it is running.
            ...
            */
            ...
            /*!
             \brief
                Checks to see whether any of the vTools in the recipe has an error (see also the \c RegisterEventObserver method to get a notification when an error state changes).
            ...
            */
            ...
            /*!
             \brief Checks to see whether a vTool is enabled.

             \pre
                 <ul>
                     <li>A recipe is loaded.</li>
                     <li>The vTool exists in the recipe.</li>
                 </ul>
              ...
              \threading
                 This method is synchronized using the lock provided by \c GetLock().
            */
            bool IsVToolEnabled(const String_t& identifier) const;

            /*!
              \brief
                 Enables or disables the vTool with the provided identifier.

              \param[in] identifier The identifier of the vTool.
              \param[in] enable Enables or disables the vTool.

              \pre
                <ul>
                    <li>A recipe is loaded.</li>
                    <li>The recipe is not started.</li>
                    <li>The recipe contains a vTool with the identifier \c identifier.</li>
                </ul>
              ...
              \threading
                 This method is synchronized using the lock provided by \c GetLock().
            */
            void SetVToolEnabled(const String_t& identifier, bool enable = true);

            /*!
             \brief
                Starts the measurement for the recipe. This should usually be done after loading the recipe and before starting it but can also be used while the recipe is running.

             \param[in] samplesPerVTool The number of samples to be collected per vTool. The default is 100,000 samples.

             \pre
                <ul>
                    <li>A recipe is loaded.</li>
                    <li>\c samplesPerVTool must not be 0.</li>
                </ul>

             \error
                The measurement is not started if an error occurred. Throws an exception if the preconditions aren't met.

             \threading
                This method is synchronized using the lock provided by \c GetLock().
            */
            void StartMeasurement(size_t samplesPerVTool = 100000u);

            /*!
             \brief
                Stops the measurement for the recipe and returns a CSV-formatted report (compatible with the TimingAnalyzer sample). This should usually be done after stopping the recipe but can also
                be used while the recipe is running. The measurement is also stopped automatically when unloading the recipe.

             \return
                A report formatted as CSV file or an empty string if the measurement wasn't running.
             ...
             \post
                <ul>
                    <li>The measurement is stopped.</li>
                </ul>

             \threading
                This method is synchronized using the lock provided by \c GetLock().
            */
            String_t StopMeasurement();
            ...
            /*!
             ...
             \return
                \c true if measurement is running or false otherwise.

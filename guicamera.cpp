    ...
    , m_invertImage( false )
{
    ...
    // Default debug saving off
    m_debugSaveEnabled = false;
    m_debugSaveInterval = 10;
}
...
void CGuiCamera::Close()
{
    ...
    StopGrab();
    // Wait for the grab thread to completely stop (avoid mutex destroy while locked)
    int wait_ms = 0;
    while (m_camera.IsGrabbing() && wait_ms < 2000)
    {
        // sleep a tiny amount, do not block too long; allow the grab thread to stop
...
void CGuiCamera::OnImageGrabbed( Pylon::CInstantCamera& camera, const Pylon::CGrabResultPtr& grabResult )
{
    ...
    if (grabResult.IsValid() && grabResult->GrabSucceeded())
    {
        ...
        // Debug: Save a preview image to file and print min/max only when enabled
        if (m_debugSaveEnabled && m_camera.IsGrabbing())
        {
            ++m_debugSaveCounter;
            if (m_debugSaveCounter >= m_debugSaveInterval)
            {
                m_debugSaveCounter = 0;
                try {
                    QMutexLocker saveLock( &m_imageLock );
                    if(!m_image.isNull()) {
                        QString filename = QString("/tmp/preview_debug_%1_%2.png").arg(m_userHint).arg((int)time(NULL));
                        m_image.save(filename);
                        // Convert to cv::Mat to get min/max
                        QImage img = m_image.convertToFormat(QImage::Format_RGB888);
                        int w = img.width();
                        int h = img.height();
                        cv::Mat mat(h, w, CV_8UC3, (uchar*)img.bits(), img.bytesPerLine());
                        double minVal, maxVal;
                        cv::minMaxLoc(mat, &minVal, &maxVal);
                        std::cout << "OnImageGrabbed() saved " << filename.toStdString() << " minVal=" << minVal << " maxVal=" << maxVal << std::endl;
                    }
                }
                catch(const std::exception &e) {
                    std::cout << "OnImageGrabbed() debug exception: " << e.what() << std::endl;
                }
            }
        }
...
void CGuiCamera::SetDebugSaveEnabled(bool enable)
{
    QMutexLocker lock(&m_MemberLock);
    m_debugSaveEnabled = enable;
    // reset counter when toggled
    m_debugSaveCounter = 0;
}

void CGuiCamera::SetDebugSaveInterval(int interval)
{
    QMutexLocker lock(&m_MemberLock);
    if (interval > 0)
        m_debugSaveInterval = interval;
}
...
/////////////////////////////////////////////////////////////
void CGuiCamera::capture(Mat &Image)
{
    ...
    //m_camera.AcquisitionMode.TrySetValue( Basler_UniversalCameraParams::AcquisitionMode_Continuous );
    m_camera.StartGrabbing(100, Pylon::GrabStrategy_LatestImageOnly);
    ...
    while (m_camera.IsGrabbing())
    {
        ...
        {
            std::cout << "SizeX: " << ptrGrabResult->GetWidth() << endl;
            std::cout << "SizeY: " << ptrGrabResult->GetHeight() << endl;
            const uint8_t* pImageBuffer = (uint8_t*) ptrGrabResult->GetBuffer();
            //Mat image(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC3, (void*)pImageBuffer);
            formatConverter.Convert(pylonImage, ptrGrabResult);
        // 将 pylon image转成OpenCV image.
            Image = Mat(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC3, (uint8_t *)pylonImage.GetBuffer()).clone();

            // Debug: compute min/max and save temporary image for inspection
            // Debug capture saves also respect the debug flag and throttle
            if (m_debugSaveEnabled && m_camera.IsGrabbing())
            {
                ++m_debugSaveCounter;
                if (m_debugSaveCounter >= m_debugSaveInterval)
                {
                    ...
                    try {
                        ...
                        std::cout << "capture() minVal=" << minVal << " maxVal=" << maxVal << std::endl;
                        // Save as timestamped file to avoid overwriting
                        std::ostringstream oss;
                        oss << "/tmp/capture_debug_" << m_userHint << "_" << time(NULL) << ".png";
                        cv::imwrite(oss.str(), Image);
                    }
                    catch(const std::exception &e) {
                        std::cout << "capture() debug exception: " << e.what() << std::endl;
                    }
                }
            }

            // Print some camera parameters to help debug in case of black frames
            try {
                if (m_camera.IsOpen()) {
                    std::cout << "ExposureTime: " << m_camera.ExposureTime.GetValue() << " Gain: " << m_camera.Gain.GetValue() << std::endl;
                    std::cout << "PixelFormat: " << m_camera.PixelFormat.GetValue() << std::endl;
                }
            }
            catch(const Pylon::GenericException &e) {
                std::cout << "camera parameter read error: " << e.GetDescription() << std::endl;
            }
...
void CGuiCamera::picshow(QString str,int x, int y)
{
